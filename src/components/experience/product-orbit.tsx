...
export type ProductOrbitProps = Omit<React.ComponentProps<"section">, "children"> & {
  ...
  primitive?: "bottle" | "capsule" | "sphere" | "torus" | "box";
...
export function ProductOrbit({
  ...
  const { ready, failed } = useWebGLStage({
    ...
    create: ({ renderer, markReady, markFailed, isDisposed, requestResize }) => {
      ...
      scene.add(new THREE.HemisphereLight("#eef7ff", "#151922", 2.4));
      const key = new THREE.DirectionalLight("#ffffff", 4.2);
      key.position.set(4, 5, 6);
      scene.add(key);
      const rim = new THREE.DirectionalLight("#5de8ff", 3.2);
      ...
      const install = (object: THREE.Object3D) => {
        product = object;
        product.scale.setScalar(resolvedModelScale);
