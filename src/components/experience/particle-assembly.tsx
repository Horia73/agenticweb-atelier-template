...
const FRAGMENT = /* glsl */ `
  ...
  void main() {
    ...
    gl_FragColor = vec4(mix(uColor, vColor, uColorMix), alpha);
  }
`;
...
export function ParticleAssembly({
  ...
  const { ready, failed } = useWebGLStage({
    ...
    signature: JSON.stringify([color, colorSrc, particleCount, pointSize, sampling, scatter, smoothing, sourceColorMix, src, trigger]),
    create: ({ renderer, markReady, markFailed, isDisposed }) => {
      ...
      const load = async () => {
        try {
          ...
          const count = Math.min(Math.max(600, particleCount), 12000);
